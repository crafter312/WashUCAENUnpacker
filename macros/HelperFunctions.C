...
void SaveRun(const int& runnum, bool overwrite = true) {
	string name = "RootFiles/Run" + std::to_string(runnum) + ".root";
	if ((access(name.c_str(), F_OK) == -1) || overwrite) {
		...
		int i = 0;
		name = "RootFiles/Run" + std::to_string(runnum) + "_";
