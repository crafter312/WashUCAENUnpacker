...
void ToTlogFit(int source, int background, int xmax=10, int ymax=50, string probe="OFF") {
	const char* histTitle = background == -1 ? "ToT vs. # of Photoelectrons" : "Background-subtracted ToT vs. # of Photoelectrons";
	const char* gates = "tot > -1";

	string fName = "run_" + to_string(source) + ".root";
	//string histDraw1 = "tot:0.0239*high-3.0075>>" + fName;
	string histDraw1 = "tot:0.02*high-2.816>>" + fName;
	TFile* f1 = new TFile(fName.c_str(), "READ");
	TTree* t1 = (TTree*)f1->Get("t");
	TH2F* h1 = new TH2F(fName.c_str(), histTitle, 100, 0, xmax, 50, 0, ymax);
	t1->Draw(histDraw1.c_str(), gates, "goff");
	...
	if(background != -1) {
		...
		TFile* f1b = new TFile(fName.c_str(), "READ");
		TTree* t1b = (TTree*)f1b->Get("t");
		TH2F* h1b = new TH2F(fName.c_str(), histTitle, 100, 0, xmax, 50, 0, ymax);
		t1b->Draw(histDraw1.c_str(), gates, "goff");
		h1->Add(h1b, -1);
	}
